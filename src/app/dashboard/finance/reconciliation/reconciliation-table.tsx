...
export function ReconciliationTable({ isAdmin, initialInvoiceId = "" }: { isAdmin: boolean; initialInvoiceId?: string }) {
  ...
  React.useEffect(() => {
    ...
    void fetchJson<ListData>(`/api/finance/reconciliation?${params}`)
      ...
      .catch((error) => { if (!cancelled) toast.error(error instanceof Error ? error.message : "加载核销记录失败") })
  ...
  const confirmVoid = async () => {
    ...
    try {
      ...
      toast.success("核销记录已撤销"); setVoiding(null); setVoidReason(""); setReload((value) => value + 1)
  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-xl border bg-card shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b px-4 py-3">
        <div><p className="text-sm font-medium">共 {total} 笔收款记录</p><p className="mt-1 text-xs text-muted-foreground">每行是一笔收款，不代表账单已结清；部分收款的账单仍会显示在“待收”中。</p></div>
        <Button variant="outline" size="sm" onClick={() => setReload((value) => value + 1)}><RefreshCcw className="mr-2 size-4" aria-hidden="true" />刷新</Button>
      </div>
    ...
    <section className="overflow-hidden rounded-xl border bg-card shadow-sm">
      {loading ? <div className="flex min-h-56 items-center justify-center gap-2 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" aria-hidden="true" />正在加载...</div> : rows.length === 0 ? <div className="flex min-h-56 flex-col items-center justify-center gap-2 text-muted-foreground"><CircleDollarSign className="size-8" aria-hidden="true" /><p className="text-sm">没有符合条件的核销记录</p></div> : <>
        <div className="hidden overflow-x-auto md:block"><Table className="text-[13px]"><TableHeader><TableRow><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">总货号</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">公司</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">货号</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">Broker 公司</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">Load #</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">账单分类</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">Invoice Number</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">支票日期</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100 text-right">本次收款</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">支票号码</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">备注</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100">状态</TableHead><TableHead className="h-10 whitespace-nowrap border-slate-800 bg-slate-950 px-3 text-[12px] font-semibold text-slate-100 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{rows.map((record) => <TableRow key={record.id} className={cn(record.voided_at && "text-muted-foreground")}><TableCell>{record.master_order_number || "—"}</TableCell><TableCell className="font-medium">{record.company}</TableCell><TableCell>{record.order_number || "—"}</TableCell><TableCell>{record.bill_to || "—"}</TableCell><TableCell>{record.broker_load_number || "—"}</TableCell><TableCell>{record.billing_category || "—"}</TableCell><TableCell className="font-medium">{record.invoice_number}</TableCell><TableCell>{date(record.check_date)}</TableCell><TableCell className="text-right font-medium tabular-nums">{money(record.check_amount)}</TableCell><TableCell>{record.check_number}</TableCell><TableCell className="max-w-44 truncate" title={record.notes || ""}>{record.notes || "—"}</TableCell><TableCell><div className="space-y-1"><StatusBadge voided={record.voided_at != null} />{record.void_reason ? <p className="max-w-48 text-xs" title={record.void_reason}>原因：{record.void_reason}</p> : null}</div></TableCell><TableCell><div className="flex justify-end gap-1">{isAdmin && !record.voided_at ? <><Button variant="ghost" size="sm" onClick={() => openEdit(record)}><Pencil className="mr-1 size-4" aria-hidden="true" />修改</Button><Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => { setVoiding(record); setVoidReason("") }}><Undo2 className="mr-1 size-4" aria-hidden="true" />撤销</Button></> : "—"}</div></TableCell></TableRow>)}</TableBody></Table></div>
