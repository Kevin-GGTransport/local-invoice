...
export function ReconciliationTable({ isAdmin, initialInvoiceId = "", hideToolbar = false }: { isAdmin: boolean; initialInvoiceId?: string; /** 工具栏整栏折叠时隐藏筛选工具卡 */ hideToolbar?: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {/* 分页：吸底浮动，长表格滚动时随时翻页（原为卡内 footer，移出卡外才能 sticky） */}
    <StickyFooter>
      <div className="flex items-center justify-between rounded-xl border bg-card px-4 py-3 text-sm"><span className="text-muted-foreground">第 {page} / {pageCount} 页</span><div className="flex gap-2"><Button variant="outline" size="sm" disabled={page <= 1 || loading} onClick={() => setPage((value) => value - 1)}><ChevronLeft className="size-4" aria-hidden="true" />上一页</Button><Button variant="outline" size="sm" disabled={page >= pageCount || loading} onClick={() => setPage((value) => value + 1)}>下一页<ChevronRight className="size-4" aria-hidden="true" /></Button></div></div>
    </StickyFooter>
