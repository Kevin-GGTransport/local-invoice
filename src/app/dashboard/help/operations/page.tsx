...
const chapters = [
  ...
  },
  {
    ...
    steps: ["在「待收」中选择公司，或输入 Invoice、货号、Load #、备注关键词查找账单，核对公司、账单编号和剩余应收；打开登记收款弹窗后再核对 Invoice 金额和已收金额。", "待收列表只包含已发、Invoice 金额大于 0 且有效已收金额小于 Invoice 金额的账单；未发和负数账单不会出现在这里。", "点击目标账单的登记收款入口，填写支票日期、本次金额和支票号码，备注可选。金额必须大于 0，最多两位小数；支票号码只允许英文字母和数字。", "点击「确认核销」。同一账单可分多次登记，每次仅填写本次实际收款，不要重复填累计已收金额。", "到「已收」查看新增记录。使用支票日期范围、公司、关键词和状态筛选查询；从某账单查看记录时会限定该账单。"],
